...
The reason being that a register can only store 1 bit. Assuming a D-type flip flop is used, this can require as little as 5 transistors as shown by Praveen Kumar Chakravarti et al (2015) \cite{Chakravarti}.
Betz (1998) \cite{betz} displays that a LUT consists of SRAM, multiplexers, input buffers, and complementers. 
Based on these components a 4-LUTs is calculated to contain about 167 transistors. However the FPGA used, the Xilinx XC6VLX240T, mainly utilizes 6-LUTs as is found by observing the area.txt file. 
...
A single SRAM cell requires six transistors  \cite{betz}, as the FPGA has 18 and 36 KB BRAM blocks, this would mean either about 108.000 or 216.000 transistors are required. 
Lastly DSPs are a a bit harder to estimate area wise. 
The user guide of the 7 Series Xilinx FGPA however reveals that the DSP has a 25 times 18 bit two's complement multiplier. 

...
Two other metrics we introduce are: ... \todo[inline]{Kun je nog iets meer info geven over performance en accuracy and waarom het belangrijk is?}
...
\begin{table*}[h]
    ...
    %S[table-format=3.1,table-space-text-post=\si{\kilo\byte}]S[table-format=1.3]S[table-format=1.3]S[table-format=1.3]
...
gshare, bi-mode, and YAGS predictors were simulated using all 8 SPEC 95 benchmarks interleaved each 60 000 instructions to simulate a high context switch environment. \todo[inline]{Volgens mij klopt deze zin niet}
